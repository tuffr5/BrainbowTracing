function skel = linkRepairV2(filename, TH)
...
for i=1:length(ep)
    ...
    tt = knnsearch(Mdl, query, 'K', 2);
    value = [node2(ep(tt(2))).comx node2(ep(tt(2))).comy node2(ep(tt(2))).comz];
    P = shortestpath(G,ep(i),ep(tt(2)));
    if isempty(P)
